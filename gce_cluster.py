...
###
# Usage simple: gce_cluster.py <project> <cluster-name> simple <no-slaves> <identity-file>
...
https://www.googleapis.com/compute/v1/projects/centos-cloud/global/images/centos-6-v20160119
https://www.googleapis.com/compute/v1/projects/centos-cloud/global/images/centos-7-v20160119
https://www.googleapis.com/compute/v1/projects/coreos-cloud/global/images/coreos-alpha-933-0-0-v20160120
https://www.googleapis.com/compute/v1/projects/coreos-cloud/global/images/coreos-beta-899-4-0-v20160121
https://www.googleapis.com/compute/v1/projects/coreos-cloud/global/images/coreos-stable-835-10-0-v20160120
https://www.googleapis.com/compute/v1/projects/debian-cloud/global/images/backports-debian-7-wheezy-v20160119
https://www.googleapis.com/compute/v1/projects/debian-cloud/global/images/debian-7-wheezy-v20160119
https://www.googleapis.com/compute/v1/projects/debian-cloud/global/images/debian-8-jessie-v20160119
https://www.googleapis.com/compute/v1/projects/google-containers/global/images/container-vm-v20151215
...
https://www.googleapis.com/compute/v1/projects/rhel-cloud/global/images/rhel-6-v20160119
https://www.googleapis.com/compute/v1/projects/rhel-cloud/global/images/rhel-7-v20160119
https://www.googleapis.com/compute/v1/projects/suse-cloud/global/images/sles-11-sp4-v20150714
https://www.googleapis.com/compute/v1/projects/suse-cloud/global/images/sles-12-sp1-v20151215
https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1204-precise-v20160114
https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1404-trusty-v20160114e
https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1504-vivid-v20160114a
https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1510-wily-v20160114
https://www.googleapis.com/compute/v1/projects/windows-cloud/global/images/windows-server-2008-r2-dc-v20160112
https://www.googleapis.com/compute/v1/projects/windows-cloud/global/images/windows-server-2012-r2-dc-v20160112
"""
image_centos_6 = 'https://www.googleapis.com/compute/v1/projects/centos-cloud/global/images/centos-6-v20141218'
image_ubuntu_14_04_v20151113 = "https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1404-trusty-v20151113"
image_ubuntu_14_04 = "https://www.googleapis.com/compute/v1/projects/ubuntu-os-cloud/global/images/ubuntu-1404-trusty-v20160114e"
...
def read_args():
    ...
    if len(sys.argv) == 8:
    ...
    elif len(sys.argv) == 6 and sys.argv[3].lower() == "simple":
        print '*** Simple ***'
        project = sys.argv[1]
        cluster_name = sys.argv[2]
        ...
        print '# Usage simple: gce_cluster.py <project> <cluster-name> simple <no-slaves> <identity-file>'
...
def launch_slaves():
    ...
    for s_id in range(1, slave_no + 1):
        command = 'gcloud compute --project "' + project + '" instances create "' + cluster_name + '-slave' + str(
            s_id) + '" --zone "' + zone + '" --machine-type "' + slave_type + '" --network "' + cluster_name + '-network" --maintenance-policy "MIGRATE" --scopes "https://www.googleapis.com/auth/devstorage.read_only" --image "' + os_image + '" --boot-disk-type "pd-standard" --boot-disk-device-name "' + cluster_name + '-s' + str(
            s_id) + 'd"'
        print command
        command = shlex.split(command)
        subprocess.call(command)
...
def launch_cluster_simple():
    ...
    ### Need to parallelize slave setup
    for s_id in range(1, slave_no + 1):
        command = 'gcloud compute --project "' + project + '" instances create "' + cluster_name + '-' + str(
            s_id) + '" --zone "' + s_zone + '" --machine-type "' + type_cpu + '" --network "default" --maintenance-policy "MIGRATE" --scopes "https://www.googleapis.com/auth/devstorage.full_control" --image "' + os_image + '" --boot-disk-type "pd-standard" --boot-disk-device-name "' + cluster_name + '-s' + str(
            s_id) + 'd"'
        print command
        command = shlex.split(command)
        subprocess.call(command)
...
def test_scp():
    global identity_file = "spectrum"
    scp_file(h, "test.txt")

def scp_file(host, filen):
    # scp -i spectrum -o 'UserKnownHostsFile=/dev/null' -o 'CheckHostIP=no' -o 'StrictHostKeyChecking no' test.txt peteperlegos@104.154.91.81:
    command = "scp -i " + identity_file + " -o 'UserKnownHostsFile=/dev/null' -o 'CheckHostIP=no' -o 'StrictHostKeyChecking no' " + filen + " " + username + "@" + host + ":"
    print command
    os.system(command)
...
def install_java_rhel(master_nodes, slave_nodes):
    ...
    if master_nodes:
        master = master_nodes[0]
        master_thread = threading.Thread(target=ssh_thread, args=(master,
                                                                  "sudo yum install -y java-1.7.0-openjdk;sudo yum install -y java-1.7.0-openjdk-devel;sudo yum groupinstall \'Development Tools\' -y"))
        master_thread.start()
        master_thread.join()

    for slave in slave_nodes:
        slave_thread = threading.Thread(target=ssh_thread, args=(slave,
                                                                 "sudo yum install -y java-1.7.0-openjdk;sudo yum install -y java-1.7.0-openjdk-devel;sudo yum groupinstall \'Development Tools\' -y"))
        slave_thread.start()
...
def start_client(slave_nodes):   ### FINISH
    print '[ Sending code to Nodes ]'
    ### scp files
    scp_file_multi(slave_nodes, filen)
    ### unzip code
    command = "tar -zxvf spectrum_gae_python.tar.gz"
    command_nodes(slave_nodes, command)

    print '[ Starting client.py on nodes ]'
    command = "nohup python spectrum_gae_python/client.py > spectrum_gae_python/client.log &"
    command_nodes(slave_nodes, command)


def scp_file_multi(nodes, filen):   ### FINISH
    # ssh and issue command in a remote node
    for node in nodes:
        slave_thread = threading.Thread(target=scp_file, args=(node, filen))
        slave_thread.start()
    slave_thread.join()
    os.system("reset")


def command_nodes(nodes, command):
    # ssh and issue command in a remote node
    for node in nodes:
        slave_thread = threading.Thread(target=ssh_thread, args=(node, command))
        slave_thread.start()
    slave_thread.join()
    os.system("reset")
...
def ssh_command(host, command):
    ...
    commands.getstatusoutput(
        "ssh -i " + identity_file + " -o 'UserKnownHostsFile=/dev/null' -o 'CheckHostIP=no' -o 'StrictHostKeyChecking no' " + username + "@" + host + " '" + command + "'")
...
def launch_main():
    ...
    # Move files
    #start_client(slave_nodes)
    #print '[ Transfering Files to Client Nodes: Waiting ' + str(waitTime) + ' Seconds ]'
    #time.sleep(waitTime)
