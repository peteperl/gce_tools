...
def launch_cluster_simple():
	...
	### Need to parallelize slave setup
	for s_id in range(1,slave_no+1):
		command = 'gcloud compute --project "' + project + '" instances create "' + cluster_name + '-' + str(s_id) + '" --zone "' + s_zone + '" --machine-type "' + type_cpu + '" --network "default" --maintenance-policy "MIGRATE" --scopes "https://www.googleapis.com/auth/devstorage.full_control" --image "' + os_image + '" --boot-disk-type "pd-standard" --boot-disk-device-name "' + cluster_name + '-s' + str(s_id) + 'd"'
		print command
		command = shlex.split(command)
		subprocess.call(command)
...
def scp_file_multi(nodes, filen):
